  ...
  if (!graph.skills.length) {
    return (
      <p className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-400 dark:border-slate-800 dark:bg-slate-900">
        No skills extracted yet — add syllabus PDFs with キーワード sections.
      </p>
    );
  }
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100">
          Skill network <span className="text-sm font-normal text-slate-400">スキルのつながり</span>
        </h2>
      ...
      <svg
        ...
        role="img"
        aria-label="Network of courses and the skills they develop"
      ...
      <p className="mt-2 text-xs text-slate-400">
        Click a skill to see talking points ・ click a course to open its syllabus ／
        スキルをクリックで詳細、科目をクリックでシラバスへ
      </p>
    </div>
  );
}
