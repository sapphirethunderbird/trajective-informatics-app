...
import { modeCounts, needsTextbook } from '../data/aggregate';
...
export function CourseCard({ course, requirement, onSetRequirement, onOpen, onRemove }: Props) {
  ...
  return (
    <div className="group relative flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
      <button
        ...
        title={t('remove')}
        aria-label={`${t('remove')} ${courseTitle(course)}`}
      >
        ✕
      </button>
      ...
      <button onClick={() => onOpen(course.id)} className="text-left">
        <div className="text-xs font-medium uppercase tracking-wide text-blue-600 dark:text-blue-400">
          {course.term} · {course.schedule} · {course.credits} {t('credits')}
        </div>
        <h3 className="mt-1 pr-6 text-base font-semibold text-slate-800 hover:text-blue-600 dark:text-slate-100">
          {courseTitle(course)}
        </h3>
        {secondaryTitle && <div className="text-sm text-slate-500 dark:text-slate-400">{secondaryTitle}</div>}
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        ...
        {needsTextbook(course) && (
          <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[11px] text-amber-800 dark:bg-amber-900/50 dark:text-amber-200">
            📕 {t('needsTextbook')}
          </span>
        )}
      </div>
      ...
      <div className="mt-3 flex h-2 gap-0.5">
        {course.weeks.map((w) => (
          <div
            ...
            title={`W${w.week} ${w.title}`}
          />
        ))}
      </div>
      <div className="mt-2 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
        <span>
          <span className="font-semibold" style={{ color: workModeColors.group }}>
            {group}
          </span>{' '}
          {t('mode.group')}／{t('mode.discussion')} ·{' '}
          <span className="font-semibold" style={{ color: workModeColors.individual }}>
            {counts.individual}
          </span>{' '}
          {t('mode.individual')}
        </span>
        <span className="text-slate-400">
          {course.weeks.length} {t('weeksLabel')}
        </span>
      </div>
      ...
      {course.parseWarnings.length > 0 && (
        <div className="mt-3 rounded-lg bg-amber-50 px-2 py-1 text-[11px] text-amber-700 dark:bg-amber-950/40 dark:text-amber-300">
          ⚠ {t('partiallyParsed')}: {course.parseWarnings.join(', ')}
        </div>
      )}
    </div>
  );
